...
async def transport_comm_daemon():
    """This function is called by the primary plugin module with no arguments.
    ...
    async with trio.open_nursery() as nursery:
        config.connection = Connection(nursery=nursery, router=network.router)
        print("connection_daemon started successfully")
        await trio.sleep_forever()


class Message:
    def __init__(self, message, from_gid):
...
class Connection(ABC):
    ...
    To provide the Channel interface, a send/recieve pair of objects should be opened
    ...
    def __init__(self, nursery, router=network.router):
        self.router = router
